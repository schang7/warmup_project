...
class DriveSquare(object):
    ...
    def turn_in_square(self, event):
        ...
        # of the square once the driving course is initiated

        # First stop the robot's forward motion if it is going in that direction,
        # then initiate angular movement for turning to the right 
        self.movement.linear.x = 0
        self.movement.angular.z = 1.0

        # Give the command some time to register and then publish
        rospy.sleep(1)
        self.movement_pub.publish(self.movement)

        # Determine time duration needed to complete 90 degree turn
        radians = math.pi/2 # to make a 90 degrees turn (in radians)
        duration_to_turn = radians / self.movement.angular.z # in sec

        # Turn for that amount of time
        rospy.sleep(duration_to_turn)

        # After the turn, stop moving in the angular direction and start moving forward again
    ...
    def run(self):
        ...
        # a specified amount of time have elapsed
